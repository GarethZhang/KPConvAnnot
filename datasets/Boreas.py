...
class BoreasDataset(PointCloudDataset):
    ...
    def __init__(self, config, set='training', balance_classes=True):
        ...
        self.ignored_labels = np.sort([0]) # TODO not sure if this would impact anything
        ...
        self.epoch_i.share_memory_() # share between workers
...
class BoreasSampler(Sampler):
    ...
    def calibration(self, dataloader, untouched_ratio=0.9, verbose=False, force_redo=False):
        ...
        if redo:
            ...
            for epoch in range(1): # TODO idk why there needs 10 iterations :)
                for batch_i, batch in enumerate(dataloader):
